import type {
    ...
    InsertOneOptions, UpdateOptions, BulkWriteOptions

} from "../../dependencies";
import { MongoClient, ObjectId } from "../../dependencies";
...
class NodeMongoWrapperCls {
    ...
    closeConnection() {
        ...
        if (this.globalConnection) {
            ...
            this.globalDB = null;
            LoggerCls.info("node-mongo-wrapper closeGlobalConnection()", "Disconnected from DB");
        }
    }

    find(_collectionName: string, _filter: Filter<Document>, _projection: Document, _limit: number, _sort: Sort): Promise<Document[]> {
    ...
    insertOne(_collectionName: string, _keyName: string, _document: Document, _session?: ClientSession): Promise<string> {
    ...
    updateOne(_collectionName: string, _filter: Filter<Document>, _updateProp: Document, _session?: ClientSession): Promise<number> {
    ...
    insertMany(_collectionName: string, _keyName: string, _documentArr: Document[], _session?: ClientSession): Promise<string[]> {


        let promObj: Promise<string[]> = new Promise((resolve, reject) => {
            if (_collectionName && _keyName && _documentArr instanceof Array && _documentArr.length) {


                for (const doc of _documentArr) {
                    const passedId = doc[_keyName] || doc["_id"];
                    doc["_id"] = passedId || (new ObjectId());
                    doc[_keyName] = doc["_id"];
                }
                ...
                this.getConnection()
                    .then((db: Db) => {
                        ...
                        db.collection(_collectionName)
                            .insertMany(_documentArr, options)
                            .then((res) => {
                                if (res && res.insertedCount && res.insertedIds) {
                                    const idsArr = Object.values(res.insertedIds).map(oid => oid.toString());
        ...
        promObj = promObj.catch((err) => {
            LoggerCls.error("node-mongo-wrapper insertMany()", err);
            throw err;
        });
    ...
    updateMany(_collectionName: string, _filter: Filter<Document>, _updateProp: Document, _session?: ClientSession): Promise<number> {
            ...
            if (_collectionName && _filter && _updateProp) {
                this.getConnection()
                    .then((db: Db) => {
                        ...
                        db.collection(_collectionName)
                            .updateMany(_filter, { $set: _updateProp }, options)
                            .then(res => {
        ...
        promObj = promObj.catch((err) => {
            LoggerCls.error("node-mongo-wrapper updateMany()", err);
            throw err;
        });
    ...
    group(_collectionName: string, _filter: Filter<Document>, _group: Document): Promise<Document[]> {
    ...
    aggregate(_collectionName: string, _pipelineArr: Document[]): Promise<Document[]> {
